...
class ReplayViewer(QMainWindow):
    def __init__(self):
        ...
        self.refresh_timer.start(5000) # Проверка новых эпох каждые 5 секунд
    ...
    def load_epochs(self):
        ...
        if len(files) == self.epoch_cb.count():
            return # Ничего нового не появилось
        ...
        # Если это первый запуск или список был пуст - загружаем первую эпоху
    ...
    def on_game_selected(self, index):
        if index < 0 or not self.current_epoch_data: return
        self.current_game_index = index
        self.current_game_moves = self.current_epoch_data["games"][index]
        self.current_move_index = 0
        
        # Если включено автовоспроизведение, продолжаем его, иначе стоп
        if self.auto_btn.isChecked():
            self.auto_play_timer.stop() # перезапуск таймера
    ...
    def start_auto_timer(self):
        speed_idx = self.speed_cb.currentIndex()
        if speed_idx == 0:
            delay = 500  # Медленная
        elif speed_idx == 1:
            delay = 150  # Нормальная
        else:
            delay = 50   # Быстрая
            
        self.auto_play_timer.start(delay)

    def toggle_auto(self):
        checked = self.auto_btn.isChecked()
        if checked:
            if self.current_move_index >= len(self.current_game_moves):
                self.next_game()
                if not self.auto_btn.isChecked(): return # Could have been turned off in next_game
